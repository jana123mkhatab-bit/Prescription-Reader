...
import { Pill, CalendarCheck, ShieldAlert, Upload, BellOff, Bell, ArrowRight } from "lucide-react";
...
export default function PatientDashboard() {
  ...
  const withInteractions = useMemo(() => (medications ? checkInteractions(medications) : []), [medications]);
  const flaggedCount = withInteractions.filter((m) => m.flaggedWith?.length > 0).length;
  ...
  return (
    <div className="flex flex-col gap-6 py-4">
      <div>
        <h1 className="font-display text-2xl font-medium text-ink">Your week at a glance</h1>
      ...
      {medications.length === 0 ? (
        ...
        <>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <StatTile icon={Pill} label="Medications" value={medications.length} />
            <StatTile icon={CalendarCheck} label="Doses left today" value={remainingToday.length} />
            <StatTile
              icon={CalendarCheck}
            ...
            <StatTile
              ...
              tone={flaggedCount > 0 ? "text-warning" : "text-ink"}
              value={flaggedCount}
            />
          </div>
          ...
          <Card>
            <CardContent className="p-5">
              <div className="mb-4 flex items-center justify-between">
                <h2 className="font-display text-base font-medium text-ink">This week&apos;s adherence</h2>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Button as={Link} href="/patient/upload" size="sm" variant="secondary">
              <Upload className="h-4 w-4" /> Upload a prescription
            </Button>
            <Link
              href="/patient/medications"
              className="flex items-center gap-1 text-sm font-medium text-brand hover:underline underline-offset-2"
            >
